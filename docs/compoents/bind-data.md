...
1. 页面数据
   - 页面数据
   - 页面全局数据
     - 放在页面 globalData 里，这样每个页面都能很方便地访问全局数据。
     - 想编辑页面全局数据请去数据中心修改。
   - 页面数据仓库
     - 数据仓库里面的数据是隔离的，要想在页面上使用就得先引入进来。
2. 样式辅助
   - 用于生成样式代码
3. 绑定记录
   - 所有组件的绑定记录，属性，样式，事件等。
...
这里的代码的意思是：页面数据 text 等于字符串"张三"，就使用#fff(白色)，不等于就使用#000(黑色)
...
以下是等于张三的效果，背景颜色变成了白的 (因为页面颜色也是白的所以看起来没效果)
![](/public/data/现在等于张三的效果.png){ width=400 }

修改数据，以下是不等于张三的效果，背景颜色变成了黑的
...
::: details 上图，左侧的迭代数据是什么？

1. 迭代值变量名

   - 代表当前循环到的这个元素的内容
   - 就像“这个货架格子里放的是什么”，例如 array 里面是放的是 1,2,3。那遍历时的 forItem 就分别等于 1，2，3。
...
![](/public/data/工作流/事件工作流.png){ width=500 }

点击开始节点，**把循环的迭代值传入到工作流中，现在 item 就是 forItem**

![](/public/data/工作流/把外部数据传入工作流中.png){ width=500 }

拖入逻辑块，写入打印 item，然后点击左上角的箭头返回。

![](/public/data/工作流/拖入逻辑块.png){ width=500 }
