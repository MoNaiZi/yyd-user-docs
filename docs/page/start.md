...
注意：navigateBack 不能携带参数。可以使用 navigateBackWithData

```js
// 返回上一个页面
uni.$GUtils.navigateBackWithData({
  delta: 1,
  data: {text: "携带的数据"},
});

// 上一个页面，只能在 onShow 里面接收
onShow(){
  if(this._backData){
    // this._backData = {text: "携带的数据"}

    // 打印
    console.log(this._backData)
...
- 打开一个还能返回的新页面时，原页面只是暂时隐藏(onHide)，执行第一次进入新页面的流程。

- 打开一个不能返回的新页面时，原页面直接被销毁(卸载 onUnload)，执行第一次进入新页面的流程。

- 点返回时，当前页面被销毁(卸载 onUnload)，上一个页面重新显示。 -->
